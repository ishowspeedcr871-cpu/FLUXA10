...
import { PdfPreviewGallery, UploadedFile } from "@/components/customer/pdf-preview-gallery";
...
export function CustomerUploadWorkspace({
  ...
  const totalPages = useMemo(() => {
    return queue.filter((f) => f.status === "accepted").reduce((acc, f) => acc + f.pages, 0);
  }, [queue]);
  const subtotal = totalPages * copies * rate;
      ...
      <div className="space-y-6">
        <Card className="overflow-hidden border border-white/10 bg-white/[0.02] shadow-glass rounded-3xl">
          ...
          <CardContent className="space-y-6">
            ...
            {/* Professional PDF Preview Gallery */}
            {queue.filter((f) => f.status === "accepted").length > 0 && (
              <div className="pt-2">
                <PdfPreviewGallery
                  files={queue
              ...
              <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-black/40 to-black/20 p-6 space-y-4">
                ...
                <div className="space-y-2.5 font-mono text-sm text-muted-foreground">
                  <div className="flex justify-between items-center text-white font-sans font-semibold">
                    ...
                    <span>{color ? "Color Printing (₹10/pg)" : "Black & White (₹2/pg)"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Total Pages:</span>
                    <span className="text-white font-semibold">{totalPages} pages</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Total Copies:</span>
                    <span className="text-white font-semibold">{copies} copies</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="border-t border-dashed border-white/10 my-2 pt-2 flex justify-between text-base font-bold text-white font-sans">
                    ...
                    <span className="text-accent-cyan text-lg">₹{subtotal.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div className="pt-2">
                  <p className="text-[10px] text-muted-foreground leading-relaxed">
                    Pricing Formula: {totalPages} pages × {copies} copies × ₹{rate}/page.
                  </p>
                </div>
              </div>
