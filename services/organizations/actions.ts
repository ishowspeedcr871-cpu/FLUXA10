...
export async function updateOrganizationSettingsAction(formData: FormData) {
  const parsed = organizationSettingsSchema.safeParse({
    ...
    currency: optionalString(formData.get("currency")) ?? "INR",
  });

  if (!parsed.success) {
    console.error("Validation failed:", parsed.error);
    redirect("/organization/settings?error=invalid_settings");
  }
...
export async function requireOrganizationOrRedirect() {
  ...
  if (!sessionData) redirect("/login?portal=organization");
  ...
  if (!profile.isOrgAdmin && !profile.isDeveloper) {
