      ...
      <div className="space-y-8 pb-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={i}
              className="bg-black/40 border border-white/10 rounded-[32px] p-4 backdrop-blur-md"
            >
        ...
        <div className="bg-black/40 border border-white/10 rounded-[40px] p-8 backdrop-blur-md space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground font-medium">
                Global tenant-isolated neural parameters
              </p>
            </div>
          </div>
          ...
          <form action={updateAiConfigurationAction} className="space-y-6">
            <div className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  defaultValue={configuration?.defaultProviderId ?? ""}
              ...
              <div className="space-y-2">
                ...
                <Select
                  name="defaultModelId"
                  defaultValue={configuration?.defaultModelId ?? ""}
                  className="bg-white/5 border-white/10 rounded-2xl h-12"
                >
              ...
              <div className="space-y-2">
                ...
                <Input
                  name="timeoutMs"
                  type="number"
                  defaultValue={configuration?.timeoutMs ?? 30000}
                  className="bg-white/5 border-white/10 rounded-2xl h-12"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { name: "isEnabled", label: "Neural Engine", checked: configuration?.isEnabled },
                {
                  ...
                  checked: configuration?.streamingEnabled,
                },
                {
                  name: "ocr_foundation",
                  label: "Vision Core",
                  checked: (configuration?.featureFlags as any)?.ocr_foundation,
                },
                {
                  name: "ai_chat_foundation",
                  label: "Dialogue Core",
                  checked: (configuration?.featureFlags as any)?.ai_chat_foundation,
                },
              ].map((flag) => (
                <label
                  key={flag.name}
                  className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/10 cursor-pointer hover:bg-white/10 transition-colors"
                >
                  <input
                    name={flag.name}
                    type="checkbox"
            ...
            <Button
              type="submit"
              className="w-full h-14 rounded-2xl bg-accent-cyan text-black font-black uppercase tracking-widest hover:bg-accent-cyan/90 transition-all"
            >
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="bg-black/40 border border-white/10 rounded-[32px] p-6 backdrop-blur-md space-y-6">
            ...
            <form action={createAiProviderAction} className="space-y-3">
              <Input
                name="key"
                placeholder="Provider ID (e.g. google-vertex)"
              ...
              <Button
                type="submit"
                size="sm"
                className="w-full rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 font-bold uppercase tracking-widest text-[10px]"
              >
                Register Provider
              </Button>
            </form>
            ...
            <div className="space-y-2">
              {dashboard.providers.map((p) => (
                ...
                >
                  ...
                  <Badge className="text-[8px] h-4 uppercase font-black tracking-widest">
                    {p.status}
                  </Badge>
                </div>
              ))}
          ...
          <div className="bg-black/40 border border-white/10 rounded-[32px] p-6 backdrop-blur-md space-y-6">
            ...
            <form action={createAiModelAction} className="space-y-3">
              ...
              <Input
                name="name"
                placeholder="Model Name (e.g. Gemini 1.5)"
                className="bg-white/5 border-white/10 rounded-xl"
              />
              <Button
                type="submit"
        ...
        <div className="space-y-4">
          <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground ml-4">
          ...
          <div className="bg-black/40 border border-white/10 rounded-[32px] overflow-hidden backdrop-blur-md">
            <div className="p-4 grid grid-cols-4 border-b border-white/10 bg-white/5">
              ...
              <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground text-right">
                Timestamp
              </span>
            </div>
            <div className="divide-y divide-white/5">
              {requests.map((r) => (
                <div
                  key={r.id}
                  className="p-4 grid grid-cols-4 items-center hover:bg-white/[0.02] transition-colors"
                >
                  <span className="text-[10px] font-bold text-accent-cyan">{r.featureKey}</span>
                  <span
                    className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full w-fit ${r.status === "COMPLETED" ? "bg-accent-cyan/10 text-accent-cyan" : "bg-accent-magenta/10 text-accent-magenta"}`}
                  >
                    {r.status}
                  </span>
